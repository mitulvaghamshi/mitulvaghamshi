...
SELECT COUNT(*) 'child patients (age < 18)' FROM patients
WHERE FLOOR(DATEDIFF(DAY, birth_date, GETDATE()) / 365.25) < 18;

--(2) GROUP - 1, SELECT - C
PRINT 'GROUP - 1, SELECT - C';
SELECT first_name, last_name, patient_height FROM patients
WHERE gender = 'F' AND patient_height >= (
    SELECT MAX(patient_height) FROM patients WHERE gender = 'F'
);

--(3) GROUP - 2, SELECT - C
PRINT 'GROUP - 2, SELECT - C';
SELECT province_id, COUNT(*) 'patients' FROM patients
...
SELECT p.patient_id, p.first_name, p.last_name, a.room, a.bed FROM patients p
JOIN admissions a ON p.patient_id = a.patient_id
WHERE a.nursing_unit_id IN (
SELECT nursing_unit_id FROM nursing_units
WHERE nursing_unit_id = '2SOUTH') AND a.discharge_date IS NULL
...
WHERE NOT EXISTS (
    SELECT l.purchase_order_id FROM purchase_order_lines l
