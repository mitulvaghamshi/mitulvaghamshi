...
_storeByte:
	STRB R0, [R1, R3]	;;; @ store Byte R0 out to R1 + offset (R3) (Fixed syntax)
	ADD  R3, R3, #1		;;; @ increment offset count
	MOV  PC, LR			;;; @ return from _storeByte

_print:
	MOV R7, #4			;;; @ setup up registers for syscall print (write)
	...
	SVC #0				;;; @ execute syscall print (Modernized from SWI)
	MOV PC, LR			;;; @ return from _print

_exit:
	MOV R0, #0			;;; @ Set exit status code to 0 (Success)
	MOV R7, #1			;;; @ setup registers for syscall exit
	SVC #0				;;; @ execute syscall exit

.data					;;; @ assembler directive used for data section
