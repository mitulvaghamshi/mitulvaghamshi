...
/// @brief A Document to be processed (printed).
class Document
{
private:
...
public:
  ...
  /// @brief Helper method definition to printout current state of the document.
  void print();
};
...
/// @brief Initialize the new document instance.
...
/// @brief Printout current status of the document,
...
/// @brief Helper procedure to handle all the underlying processing,
///        printing, and queue management logic.
///        Any changes made inside this function will be
///        automatically reflected to original data.
/// @param queue   - Reference to active queue.
/// @param chance  - Chances of adding new document.
/// @param time    - Current time status.
/// @param queueId - Which queue currrently processing.
void process(queue<Document> *queue, int chance, int time, int queueId)
{
  ...
  cout << "[Time: " << setw(2) << to_string(time) << ": PrintQueue " << queueId
       << "]: Processing: ";
...
int main(void)
{
  ...
  // It is the library implemention's selection of a generator that provides at
  // least acceptable engine behavior for relatively casual, inexpert, and/or
  // lightweight use.
  ...
  while (flag == true && time++ < 60)
  {
    ...
    if (queue1.size() > 0)
    {
      ...
      // Param: Queue reference, adding chances, current time, queue number
  ...
  if (time < 60)
  {
    cout << "Finished processing all documents. Time saved: "
         << (60 - time) << " Seconds." << endl;
  }
  else
  {
    cout << "Processing timed out. Unprocessed documents: [Queue 1: "
         << queue1.size() << " Docs, Queue 2: " << queue2.size() << " Docs] "
         << endl;
  }
  return 0;
}
