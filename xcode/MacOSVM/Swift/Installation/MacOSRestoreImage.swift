...
import Virtualization
    ...
    class MacOSRestoreImage: NSObject {
        ...
        public func download(completionHandler: @escaping () -> Void) {
            ...
            VZMacOSRestoreImage.fetchLatestSupported {
        ...
        private func downloadRestoreImage(
            ...
            let downloadTask = URLSession.shared.downloadTask(
                with: restoreImage.url
                ...
                guard
                    ...
                    fatalError(
                        "Failed to move downloaded restore image to \(restoreImageURL)."
                    )
                }
            ...
            downloadObserver = downloadTask.progress.observe(
                \.fractionCompleted,
                options: [.initial, .new]
            ) { (progress, change) in
                NSLog(
                    "Restore image download progress: \(change.newValue! * 100)."
                )
            }
            downloadTask.resume()
        }
