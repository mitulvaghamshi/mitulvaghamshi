/* Abstract: The app delegate that sets up and starts the virtual machine. */

import Virtualization

let vmBundlePath = NSHomeDirectory() + "/Developer/source/vm.bundle/"
...
class AppDelegate: NSObject {
    ...
    private func createMainDiskImage() {
        ...
        guard
            let mainDiskFileHandle = try? FileHandle(
                forWritingTo: URL(fileURLWithPath: mainDiskImagePath)
            )
        else {
...
extension AppDelegate {
        ...
        DispatchQueue.main.async {
            ...
            self.virtualMachine.start(completionHandler: { (result) in
                ...
                case .failure(let error):
                    fatalError(
                        "Virtual machine failed to start with error: \(error)"
                    )
    ...
    func createVirtualMachine() {
        ...
        virtualMachineConfiguration.consoleDevices = [
...
extension AppDelegate {
    ...
    private func createBlockDeviceConfiguration()
    ...
    private func computeCPUCount() -> Int {
        ...
        var virtualCPUCount =
        totalAvailableCPUs <= 1 ? 1 : totalAvailableCPUs - 1
        virtualCPUCount = max(
            virtualCPUCount,
    ...
    private func createAndSaveMachineIdentifier() -> VZGenericMachineIdentifier
    {
       ...
       try! machineIdentifier.dataRepresentation.write(
        to: URL(fileURLWithPath: machineIdentifierPath)
       )
    ...
    private func retrieveMachineIdentifier() -> VZGenericMachineIdentifier {
        ...
        guard
            let machineIdentifierData = try? Data(
                contentsOf: URL(fileURLWithPath: machineIdentifierPath)
            )
        else {
        ...
        guard
            let efiVariableStore = try? VZEFIVariableStore(
                creatingVariableStoreAt: URL(
                    fileURLWithPath: efiVariableStorePath
                )
            )
        else {
    ...
    private func retrieveEFIVariableStore() -> VZEFIVariableStore {
        ...
        return VZEFIVariableStore(
            url: URL(fileURLWithPath: efiVariableStorePath)
        )
    }

    private func createUSBMassStorageDeviceConfiguration()
    -> VZUSBMassStorageDeviceConfiguration
    {
       guard
        let intallerDiskAttachment =
       ...
       return VZUSBMassStorageDeviceConfiguration(
        attachment: intallerDiskAttachment
       )
    }

    private func createNetworkDeviceConfiguration()
    ...
    private func createGraphicsDeviceConfiguration()
    ...
    private func createInputAudioDeviceConfiguration()
    ...
    private func createOutputAudioDeviceConfiguration()
    -> VZVirtioSoundDeviceConfiguration
    {
...
extension AppDelegate: VZVirtualMachineDelegate {
    ...
    func virtualMachine(
        ...
        print(
            "Virtual machine did stop with error: \(error.localizedDescription)"
        )
        exit(-1)
    }
    ...
    func virtualMachine(
        ...
        print(
            "Netowrk attachment was disconnected with error: \(error.localizedDescription)"
        )
    }
}
...
extension AppDelegate: NSApplicationDelegate {
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        // If "VM.bundle" doesn't exist, the sample app tries to create
