/* Abstract: A command-line utility that runs Linux in a virtual machine. */
...
do {
    ...
    print("[Config]: \(error.localizedDescription)")
    exit(EXIT_FAILURE)
}
...
extension Delegate: VZVirtualMachineDelegate {
    func guestDidStop(_ virtualMachine: VZVirtualMachine) {
        print("[Stop]: The guest shut down. Exiting...")
        exit(EXIT_SUCCESS)
    }
}
...
func createBootLoader(kernelURL: URL, initialRamdiskURL: URL) -> VZBootLoader {
    print("Kernel: \(CommandLine.arguments[1])")
    print("Randisk: \(CommandLine.arguments[2])")
...
func printUsageAndExit() -> Never {
    print(
        "Usage: \(CommandLine.arguments[0]) <kernel-path> <initial-ramdisk-path>"
    )
    exit(EX_USAGE)
}
