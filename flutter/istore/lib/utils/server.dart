...
abstract class Server {
  static Iterable<Product> getProductsBy(category) => category == Category.all
      ? _allProducts
      : _allProducts.where((p) => p.category == category);

  static Iterable<Product> search(category, term) => term == ''
      ? getProductsBy(category)
      : getProductsBy(category)
          .where((p) => p.name.toLowerCase().contains(term.toLowerCase()));

  static Product getProductBy(id) => _allProducts.firstWhere((p) => p.id == id);

  static int availableProducts = _allProducts.length;
