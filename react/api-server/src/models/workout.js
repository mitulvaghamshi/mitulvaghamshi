...
const { saveToDatabase } = require("../database/utils").default;
  ...
  try {
    ...
    if (filter.name) {
      return workouts.filter((workout) => {
        return workout.mode.toLowerCase().includes(filter.name);
      });
    }
    if (filter.mode) {
      return workouts.filter((workout) => {
        return workout.mode.toLowerCase().includes(filter.mode);
      });
    }
    return workouts;
  } catch (error) {
...
const updateWorkout = async (workoutId, changes) => {
  try {
