...
|  205 | Reset Content                    | The request was processed and the client should reset the document view                                                                                           |
...
|  214 | Transformation Applied           | This status code indicates that a transformation has been applied to the representation                                                                           |
|  226 | Instance Manipulations (IM) Used | This indicates that server is returning a delta to the GET request that it received                                                                               |
...
|  308 | Permanent Redirect | This indicates that the resource requested has been definitively moved to the URL given by the `Location` headers                                                                            |
...
|  419 | Page Expired                                     | Primarilu used by the Laravel PHP framework to indicate a CSRF token mismatch                                                                                                                                                    |
|  420 | Enhance Your Calm (Twitter)                      | The client is being rate-limited; a reference to cannabis culture                                                                                                                                                                |
|  421 | Misdirected Request                              | This ndicates that the request was directed to a server that is not able to produce a response                                                                                                                                   |
...
|  425 | Too Early                                        | This indicates that the server is unwilling to risk processing a request that might be replayed, which creates the potential for a replay attack                                                                                 |
...
|  428 | Precondition Required                            | This indicates that the server requires the request to be conditional. Typically, this means that a required precondition header, such as `If-Match`, is missing                                                                 |
...
|  495 | SSL Certificate Error                            | Specific to nginx and is returned by the server to indicate that there has been an error with the client certificate verification                                                                                                |
|  496 | SSL Certificate Required                         | Specific to nginx and is returned by the server to indicate that the client has not presented the required certificate                                                                                                           |
|  497 | HTTP Request Sent to HTTPS Port                  | Specific to nginx and is returned by the server to indicate that a regular HTTP request has been sent to the HTTPS port                                                                                                          |
|  498 | Token expired/invalid                            | Specific to ArcGIS and indicates that the HTTP request included a token but it was not accepted                                                                                                                                  |
|  499 | Client Closed Request                            | Specific to nginx and indicates that the client has closed the HTTP Connection in advance of the server completing the task. As such, the final response cannot be sent and this status code is only available in the nginx logs |
|  499 | Token Required                                   | Specific to ArcGIS and indicates that the server is expecting a token in the HTTP request but it was not submitted                                                                                                               |
...
|  510 | Not Extended                    | This indicates that any described extensions are not supported for the request, when request contains an extension declaration, that describes the extension to be used                                                           |
|  511 | Network Authentication Required | This status is not generated by origin servers, but by intercepting proxies that control access to the network                                                                                                                    |
|  521 | Web Server Is Down              | Specific to Cloudflare and occurs when the origin web server refuses connections from Cloudflare                                                                                                                                  |
|  522 | Connection Timed Out            | Specific to Cloudflare and occurs when Cloudflare times out contacting the origin web server                                                                                                                                      |
|  523 | Origin Is Unreachable           | Specific to Cloudflare and occurs when Cloudflare cannot contact your origin web server. This typically occurs when a network device between Cloudflare and the origin web server doesn’t have a route to the origin’s IP address |
|  525 | SSL Handshake Failed            | Specific to Cloudflare and indicates that the SSL handshake between Cloudflare and the origin web server failed                                                                                                                   |
|  530 | Site Frozen                     | Specific to Cloudflare and Pantheon and indicates an issue occured between a user and a website server                                                                                                                            |
|  599 | Network Connect Timeout Error   | Used by some proxies to signal a network connect timeout behind the proxy to a client in front of the proxy                                                                                                                       |
