...
extension Problem1 on Elevation {
  MapEntry<int, int> findLowestElevation(List<List<int>> data) {
    ...
    return MapEntry(minElevation, minElevationCount);
    // Lowest elevation: 20119, and it occurs: 4 times.
  }
}

/// Find all the local peaks that is grater then or equal to
/// value [DataConfig.minPeak], and that is not contained
/// in the given exclusion radius [DataConfig.exRadius].
///
...
extension Problem2 on Elevation {
  List<Peak> findLocalPeaks(List<List<int>> data) {
    final list = List<Peak>.empty(growable: true);
...
extension Problem3 on Elevation {
  (Peak, Peak) findClosestPeaks(List<Peak> peaks) {
    late (Peak, Peak) pair;
...
extension Problem4 on Elevation {
  MapEntry<int, int> findMostCommonHeight(List<List<int>> data) {
    // Create a list to hold maximum possible value as an index from the dataset.
