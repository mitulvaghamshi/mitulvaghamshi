...
class Store {
  ...
  void populateStore(Iterator<String> entries) {
    ...
    while (i < config.totalCustomer && entries.moveNext()) {
      ...
      var line =
          customer.cartSize > config.expressItems ? config.expressLines : 0;
  ...
  void printQueueInfo() {
    config.timeToEmpty = 0;
    for (var i = 0; i < config.totalLines; i++) {
      final line = _checkoutLines[i];
      if (line.waitTime > config.timeToEmpty) {
        config.timeToEmpty = line.waitTime;
      }
      print('Checkout #${i + 1} '
          '[${i < config.expressLines ? 'Express' : 'Regular'} '
          '(time: ${line.waitTime}s)]\n$line');
    }
    print('Total time to serve all customers '
        'in store: ${config.timeToEmpty}s\n');
  }
  ...
  void serveCustomers() {
    ...
    for (var i = 0; i < config.timeToEmpty; i++) {
      var status = queueStatus(i);
      if (i > 0 && i % 30 == 0) buffer.writeln(status);
    }
    // Check for any leftover customer.
    if (config.timeToEmpty % 30 != 0) {
      buffer.writeln(queueStatus(config.timeToEmpty));
    }
    print(buffer);
  }
}
...
extension on Store {
  ...
  String queueStatus(int time) {
    ...
    for (var line in _checkoutLines) {
      if (!line.queue.isEmpty && line.queue.peek.isDone) line.queue.remove();
