...
```sh
shar $(find . -type f -iname "*.sh" -print) > scripts.bak

tar -czf - file1 file2 | { if cd "/destination"; then tar -xzf - ; fi; }

seq -f "test/File%g\ \[dfxdxx53\]\.m4a" 10 | xargs touch
...
sed -En 's/^[ ]*"clone_url":[ ]?"(.*)",/\1/p' repos_fmt.json
